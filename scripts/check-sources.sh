...
RESUKI_COMMIT=$(_curl "https://api.github.com/repos/ReSukiSU/ReSukiSU/commits/main" \
  | jq -r '.sha // "unknown"' 2>/dev/null || echo "unknown")
[ -z "$RESUKI_COMMIT" ] && RESUKI_COMMIT="unknown"
echo "ReSukiSU main: ${RESUKI_COMMIT:0:8}"
...

{
  ...
  echo "CHECK_RESUKI_COMMIT=$RESUKI_COMMIT"
