...
HASH_FILE=$(find ./artifacts -name "kernel_hash.txt" | head -1)
KERNEL_HASH=$([ -f "$HASH_FILE" ] && tr -d "[:space:]" < "$HASH_FILE" || echo "unknown")
...
_sf=$(find ./artifacts -name "resuki_ksu_tag.txt" | head -1)
RESUKI_TAG=$([ -f "$_sf" ] && cat "$_sf" | tr -d '[:space:]' || echo "unknown")
...
if [ "$BUILD_TYPE" = "testing" ]; then
  RELEASE_TAG="shibuya-${KERNEL_HASH}-testing"
  IS_PRERELEASE="true"
else
  RELEASE_TAG="shibuya-${KERNEL_HASH}"
  IS_PRERELEASE="false"
fi

RELEASE_NAME="${KERNEL_UNAME}"
